...
describe('Environment Validation Script', () => {
  ...
  describe('Environment Variable Definitions', () => {
    ...
    it('should validate NEXT_PUBLIC_SERVER_URL correctly', () => {
      ...
      expect(definition.validate('ftp://invalid')).toContain('Must be a valid URL');
    ...
    it('should validate OpenAI API key format', () => {
      ...
      // Invalid format
      expect(definition.validate('invalid-key')).toContain('Must start with sk-');
    ...
    it('should validate database URL format', () => {
      ...
      expect(definition.validate('postgres://user:pass@localhost/db')).toBe(true);
      expect(definition.validate('postgresql://user:pass@localhost/db')).toBe(true);
      
      // Invalid URLs
      expect(definition.validate('mysql://user:pass@localhost/db')).toContain('Must be a valid PostgreSQL');
    ...
    it('should validate OTEL sampling ratio', () => {
      ...
      expect(definition.validate('invalid')).toContain('Must be between 0 and 1');
    });
  });
...
BROWSERBASE_API_KEY=example_key
    ...
    it('should handle deployment-specific validation', () => {
      ...
      mockFs.existsSync.mockReturnValue(true);
      mockFs.readFileSync.mockReturnValue('NEXT_PUBLIC_SERVER_URL=https://app.pages.dev');
      ...
      expect(consoleSpy.log).toHaveBeenCalledWith(
        expect.stringContaining('🎯 Validating for deployment target: Cloudflare Pages')
      );
