...
} from "./voice-conversation-button";
...
describe("useVoiceConversation hook", () => {
	it("provides correct initial state", () => {
		let hookResult:
			| {
					...
					handleVoiceMessage: (audioData: ArrayBuffer) => void;
			  }
			| undefined;

		function TestComponent() {
			hookResult = useVoiceConversation();
			return null;
		}

		render(<TestComponent />);

		expect(hookResult?.isInConversation).toBe(false);
		expect(hookResult?.conversationState.isActive).toBe(false);
		expect(hookResult?.conversationState.isConnected).toBe(false);
		expect(hookResult?.error).toBe(null);
	});

	it("updates state when conversation starts", () => {
		let hookResult:
			| {
					isInConversation: boolean;
					conversationState: VoiceConversationState;
		...
		act(() => {
			hookResult?.handleConversationStart();
		});
		expect(hookResult?.isInConversation).toBe(true);
		expect(hookResult?.conversationState.isActive).toBe(true);
	});

	it("resets state when conversation ends", () => {
		let hookResult:
			| {
					isInConversation: boolean;
					conversationState: VoiceConversationState;
		...
		act(() => {
			hookResult?.handleConversationEnd();
		});
		expect(hookResult?.isInConversation).toBe(false);
		expect(hookResult?.conversationState.isActive).toBe(false);
	});
	...
	it("handles errors correctly", () => {
		...
		expect(hookResult?.error).toEqual(error);
		expect(hookResult?.isInConversation).toBe(false);
	});
});
