...
export function SandboxSelector({ className }: SandboxSelectorProps) {
  const { useLocalSandbox, setUseLocalSandbox, canUseLocal, checkLocalAvailability } = useSandboxStore();
  ...
  return (
    <div className={`flex items-center gap-3 ${className}`}>
      <div className="flex items-center gap-2 text-sm">
        <Cloud className={`h-4 w-4 ${!useLocalSandbox ? 'text-blue-500' : 'text-muted-foreground'}`} />
        <span className={!useLocalSandbox ? 'text-foreground' : 'text-muted-foreground'}>
          Cloud
        </span>
      </div>
      ...
      <div className="flex items-center gap-2 text-sm">
        <Monitor className={`h-4 w-4 ${useLocalSandbox ? 'text-green-500' : 'text-muted-foreground'}`} />
        <span className={useLocalSandbox ? 'text-foreground' : 'text-muted-foreground'}>
          Local
        </span>
      </div>
...
export function useSandboxPreference() {
  ...
  return {
    ...
    canUseLocal,
    preferenceText: useLocalSandbox && canUseLocal ? "Local (Dagger)" : "Cloud (E2B)",
  };
}