...
export class GeminiTTSService {
	...
	async generateMultiSpeakerAudio(
		...
		try {
			...
			const speakerVoiceConfigs = speakerConfig.speakers.map((speaker) => ({
			...
			const response = await model.generateContent({
				...
				generationConfig: {
					// Note: Audio generation is experimental and may not be available in all SDK versions
					...(process.env.GEMINI_AUDIO_ENABLED === "true" && {
