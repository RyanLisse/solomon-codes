...
export class ProductionMonitoringService {
  ...
  private getProductionConfig(): ProductionMonitoringConfig {
    ...
    return {
      metrics: {
        ...
        interval: parseInt(process.env.METRICS_INTERVAL || '30000'), // 30 seconds
      ...
      alerting: {
        ...
        channels: {
          email: process.env.ALERT_EMAIL ? process.env.ALERT_EMAIL.split(',') : undefined,
      ...
      tracing: {
        enabled: isProduction,
        samplingRate: parseFloat(process.env.TRACE_SAMPLING_RATE || '0.1'), // 10%
        exportInterval: parseInt(process.env.TRACE_EXPORT_INTERVAL || '5000'), // 5 seconds
      },
      logging: {
        ...
        retention: parseInt(process.env.LOG_RETENTION_DAYS || '30'), // 30 days
      },
    };
  }
  ...
  private initialize(): void {
    if (!this.config.metrics.enabled) {
      this.logger.info('Production monitoring disabled for non-production environment');
      return;
    }
  ...
  private async checkAlertThresholds(metrics: PerformanceMetrics): Promise<void> {
    ...
    if (metrics.errorRate > this.config.alerting.thresholds.errorRate) {
      alerts.push({
        ...
        timestamp: Date.now(),
        severity: metrics.errorRate > 0.1 ? AlertSeverity.CRITICAL : AlertSeverity.HIGH,
    ...
    if (metrics.responseTime.p95 > this.config.alerting.thresholds.responseTime) {
      alerts.push({
        id: `response-time-${Date.now()}`,
        timestamp: Date.now(),
        severity: metrics.responseTime.p95 > this.config.alerting.thresholds.responseTime * 2 
    ...
    if (metrics.memory.percentage > this.config.alerting.thresholds.memoryUsage) {
      alerts.push({
        id: `memory-usage-${Date.now()}`,
        timestamp: Date.now(),
        severity: metrics.memory.percentage > 0.95 ? AlertSeverity.CRITICAL : AlertSeverity.HIGH,
  ...
  private async sendEmailAlert(alert: Alert): Promise<void> {
    ...
    this.logger.info('Email alert would be sent', { alert, recipients: this.config.alerting.channels.email });
  }
