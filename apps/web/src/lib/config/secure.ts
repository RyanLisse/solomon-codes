...
export class SecureConfigService {
   ...
   */
  private validateApiKeyFormat(key: string, service: keyof typeof API_KEY_PATTERNS): boolean {
    ...
    try {
      const rawConfig = {
        ...
        telemetry: {
          ...
          samplingRatio: process.env.OTEL_SAMPLING_RATIO ? parseFloat(process.env.OTEL_SAMPLING_RATIO) : undefined,
          timeout: process.env.OTEL_TIMEOUT ? parseInt(process.env.OTEL_TIMEOUT, 10) : undefined,
        },
      };
      ...
      if (error instanceof z.ZodError) {
        ...
        throw new SecurityConfigError(`Configuration validation failed: ${issues}`);
      }
      throw new SecurityConfigError("Failed to load secure configuration", error);
    }
  }
  ...
  private performSecurityValidations(config: SecureConfig): void {
    const validations = [
      ...
      {
        condition: !this.validateApiKeyFormat(config.browserbase.apiKey, "browserbase"),
        error: "Browserbase API key format is invalid",
      },
      {
        condition: config.app.environment === "production" && config.app.url.includes("localhost"),
        error: "Production environment cannot use localhost URL",
      },
      {
        condition: config.app.environment === "production" && !config.telemetry.endpoint,
    ...
    if (failures.length > 0) {
      throw new SecurityConfigError(`Security validation failed: ${failures.map(f => f.error).join(", ")}`);
    }
  }
  ...
  healthCheck(): { status: "healthy" | "unhealthy"; issues: string[] } {
    try {
      ...
      const issues = error instanceof SecurityConfigError 
        ? [error.message]
