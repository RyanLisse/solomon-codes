...
export function getElectricConfig(): ElectricConfig {
	return {
		...
		retryAttempts: parseInt(process.env.ELECTRIC_RETRY_ATTEMPTS || "3", 10),
		retryDelay: parseInt(process.env.ELECTRIC_RETRY_DELAY || "1000", 10),
	};
}
 ...
 */
export async function createElectricClient(customConfig?: Partial<ElectricConfig>) {
	const config = customConfig ? { ...getElectricConfig(), ...customConfig } : getElectricConfig();
	...
	try {
		...
		electricClient = {
			...
			sync: async (table: string) => true,
			subscribe: async (table: string, callback: Function) => `subscription-${table}-${Date.now()}`,
			unsubscribe: async (subscriptionId: string) => true,
			isConnected: () => !isOffline,
		};
		
		return electricClient;
	} catch (error) {
		throw new Error(`Failed to create Electric client: ${error instanceof Error ? error.message : String(error)}`);
	}
}
 ...
 */
export async function unsubscribeFromTableChanges(subscriptionId: string): Promise<boolean> {
	if (!electricClient) {
		return false;
	}
 ...
 */
export function resolveEnvironmentConflict(conflict: ConflictData<Environment>): Environment {
	const { local, remote } = conflict;
