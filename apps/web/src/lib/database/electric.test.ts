...
describe("ElectricSQL Real-time Synchronization", () => {
	...
	describe("Subscription Management", () => {
		it("should subscribe to table changes", async () => {
			...
			const subscriptionId = await subscribeToTableChanges("tasks", mockCallback);
			
			expect(subscriptionId).toBe("subscription-id");
			expect(mockElectricClient.subscribe).toHaveBeenCalledWith("tasks", mockCallback);
		});
		...
		it("should unsubscribe from table changes", async () => {
			...
			expect(mockElectricClient.unsubscribe).toHaveBeenCalledWith("subscription-id");
		});
	});
	...
	describe("Conflict Resolution", () => {
		it("should handle conflict resolution for tasks", async () => {
			const conflictData = {
				local: { id: "1", title: "Local Task", updatedAt: new Date("2023-01-01") },
				remote: { id: "1", title: "Remote Task", updatedAt: new Date("2023-01-02") },
			};
		...
		it("should handle conflict resolution for environments", async () => {
			const conflictData = {
				local: { id: "1", name: "Local Env", updatedAt: new Date("2023-01-02") },
				remote: { id: "1", name: "Remote Env", updatedAt: new Date("2023-01-01") },
			};
		...
		it("should use custom conflict resolution strategy", async () => {
			...
			const customResolver = vi.fn().mockReturnValue({ id: "1", title: "Merged Task" });
			
			const { resolveConflictWithStrategy } = await import("./electric");
			
			const resolved = resolveConflictWithStrategy(conflictData, customResolver);
	...
	describe("Offline Support", () => {
		...
		it("should queue operations when offline", async () => {
			const { setOfflineMode, queueOperation, getQueuedOperations } = await import("./electric");
		...
		it("should sync queued operations when coming online", async () => {
			...
			const { setOfflineMode, queueOperation, syncQueuedOperations } = await import("./electric");
			
			setOfflineMode(true);
			queueOperation({ type: "insert", table: "tasks", data: { title: "Task 1" } });
			queueOperation({ type: "update", table: "tasks", data: { id: "1", title: "Updated Task" } });
