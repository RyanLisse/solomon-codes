...
import { tasks, environments } from "./schema";
import type { 
	Task, 
	Environment, 
	MigrationStatus, 
...
export function validateTaskData(task: any): ValidationResult {
	...
	if (!task.status || !["IN_PROGRESS", "DONE", "MERGED"].includes(task.status)) {
 ...
 */
export function validateEnvironmentData(environment: any): ValidationResult {
	const errors: string[] = [];
	...
	if (!environment.githubOrganization || typeof environment.githubOrganization !== "string") {
		errors.push("Environment githubOrganization is required and must be a string");
	}
	...
	if (!environment.githubRepository || typeof environment.githubRepository !== "string") {
		errors.push("Environment githubRepository is required and must be a string");
	}
 ...
 */
function transformTaskForDatabase(task: any): NewTask {
	return {
		id: task.id,
		title: task.title,
			...
			try {
				...
				if (validateData) {
					...
					if (!validation.isValid) {
						result.errors.push(`Task ${task.id}: ${validation.errors.join(", ")}`);
						continue;
					}
				}
 ...
 */
export async function migrateEnvironments(options: MigrationOptions = {}): Promise<MigrationResult> {
	const { validateData = true } = options;
	const result: MigrationResult = {
		success: false,
		migratedCount: 0,
		...
		for (const environment of localEnvironments) {
			try {
				...
				if (validateData) {
					...
					if (!validation.isValid) {
						result.errors.push(`Environment ${environment.id}: ${validation.errors.join(", ")}`);
						continue;
					}
				}
				...
				result.errors.push(`Environment ${environment.id}: ${error instanceof Error ? error.message : String(error)}`);
			}
		}
 ...
 */
export async function performFullMigration(options: MigrationOptions = {}): Promise<MigrationResult> {
	...
	try {
		...
		result.totalMigrated = (result.totalMigrated || 0) + taskResult.migratedCount;
		
		// Step 2: Migrate environments
		updateMigrationStatus({ progress: 50, currentStep: "Migrating environments" });
		const environmentResult = await migrateEnvironments(options);
		result.errors.push(...environmentResult.errors);
		result.totalMigrated = (result.totalMigrated || 0) + environmentResult.migratedCount;
		
		// Step 3: Validation
		updateMigrationStatus({ progress: 75, currentStep: "Validating migration" });
		// Additional validation could be performed here
		
		// Step 4: Cleanup
		if (cleanupAfterMigration && result.errors.length === 0) {
			updateMigrationStatus({ progress: 90, currentStep: "Cleaning up localStorage" });
