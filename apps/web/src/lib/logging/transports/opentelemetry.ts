...
export class OpenTelemetryTransport extends winston.transports.Stream {
  ...
  constructor(options: winston.transports.StreamTransportOptions = { stream: process.stdout }) {
    super({
      ...options,
      stream: options.stream || process.stdout, // Provide required stream property
    });
...
export function createOpenTelemetryLogger(
  ...
  if (enableOTLP && config.isEnabled) {
    transports.push(
      new OpenTelemetryTransport({ level, stream: process.stdout }),
    );
  }
...
export class GlobalErrorHandler {
  ...
  private handleUnhandledRejection(
    ...
    if (
      ...
      if (typeof process !== "undefined" && process.exit) {
        ...
        throw new Error(error.message || "Critical unhandled rejection");
      }
    }
  }
  ...
  private handleUncaughtException(error: Error): void {
    ...
    if (typeof process !== "undefined" && process.exit) {
      ...
      throw new Error(error.message || "Uncaught exception");
    }
  }
