...
class RealLettaClient implements LettaClient {
	...
	agents = {
		create: async (config: {
			...
			return {
				id: response.id,
				name: response.name,
				memoryBlocks: response.memory_blocks || config.memoryBlocks,
		...
		messages: {
			create: async (
				...
				return {
					messages: response.messages.map((msg: LettaApiMessage) => ({
						messageId: msg.id,
						agentId: agentId,
		...
		get: async (agentId: string): Promise<LettaAgent> => {
			...
			return {
				id: response.id,
				name: response.name,
				memoryBlocks: response.memory_blocks || [],
				model: response.model,
				embedding: response.embedding,
				tools: response.tools || [],
			};
		...
		list: async (): Promise<LettaAgent[]> => {
			const response = await this.makeRequest("/agents");
			return response.map((agent: LettaApiAgent) => ({
				id: agent.id,
				name: agent.name,
	...
	tools = {
		list: async (): Promise<Array<{ name: string; description: string }>> => {
			...
			return response.map((tool: LettaApiTool) => ({
				name: tool.name,
