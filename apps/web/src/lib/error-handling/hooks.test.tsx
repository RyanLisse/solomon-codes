...
describe("useFormValidation", () => {
		...
		it("should pass validation with valid values", () => {
			...
			act(() => {
				Object.keys(validationRules).forEach((field) => {
					result.current.handleFieldBlur(field as keyof TestForm);
				});
			});
	...
	describe("Edge Cases", () => {
		it("should handle fields without validation rules", () => {
			...
			act(() => {
				result.current.handleFieldBlur("extraField" as keyof TestForm);
			});

			expect(
				result.current.getFieldError("extraField" as keyof TestForm),
			).toBeUndefined();
		});
