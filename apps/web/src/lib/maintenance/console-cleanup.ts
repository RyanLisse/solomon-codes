...
const logger = createContextLogger("console-cleanup");
 ...
 */
export function analyzeConsoleUsage(filePath: string, content: string): ConsoleIssue[] {
  const issues: ConsoleIssue[] = [];
  
  // Skip allowed files
  if (ALLOWED_CONSOLE_FILES.has(filePath) || 
    ...
    if (consoleMatch) {
      ...
      switch (type) {
        ...
        case "info":
          ...
          break;
        case "log":
          if (line.includes("debug") || line.includes("Dev") || line.includes("development")) {
      ...
      if (line.includes("isAuthenticated") || 
          line.includes("mock") || 
...
export function generateConsoleCleanupReport(): string {
  ...
  report += "## Overview\n\n";
  report += "This report identifies console statements that should be replaced with proper structured logging for production readiness.\n\n";
  ...
  report += "1. **Import logging:** Add `import { createContextLogger } from '@/lib/logging/factory';`\n";
  report += "2. **Create logger:** Add `const logger = createContextLogger('component-name');`\n";
  report += "3. **Replace calls:** Use appropriate logger method (error, warn, info, debug)\n";
  report += "4. **Add context:** Include relevant metadata in logging calls\n";
  report += "5. **Test thoroughly:** Ensure logging works in all environments\n\n";
