...
import { todoTracker, type TodoItem } from "./todo-tracker";
...
const SCANNABLE_EXTENSIONS = [
  ".js", ".jsx", ".ts", ".tsx", ".json", ".md", ".yml", ".yaml", ".env"
];
 ...
 */
function determinePriority(type: string, content: string, filePath: string): "low" | "medium" | "high" | "critical" {
 ...
 */
async function scanFile(filePath: string, rootPath: string): Promise<CodeIssue[]> {
  try {
 ...
 */
async function scanDirectory(dirPath: string, rootPath: string): Promise<string[]> {
  const files: string[] = [];
 ...
 */
export async function runTodoAudit(rootPath: string = process.cwd()): Promise<AuditResults> {
...
export function registerAuditResults(results: AuditResults): void {
  ...
  for (const issue of results.issues) {
    ...
    if (issue.type === "CONSOLE" && isAllowedConsoleUsage(issue.file, issue.description)) {
      continue;
    }
    
    try {
      todoTracker.registerTodo({
        type: issue.type === "CONSOLE" ? "NOTE" : (issue.type as TodoItem["type"]),
      ...
      logger.warn("Failed to register TODO item", { issue, error });
    }
  }
 ...
 */
export function generateProductionReadinessReport(results: AuditResults): string {
  ...
  if (productionReadiness.highPriority.length > 0) {
    ...
    for (const issue of productionReadiness.highPriority.slice(0, 10)) { // Limit to first 10
