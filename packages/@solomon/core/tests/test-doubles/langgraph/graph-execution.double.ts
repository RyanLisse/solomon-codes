...
export interface MockGraphExecutionHelpers {
	...
	givenNodeExecutionReturns(nodeId: string, output: unknown): void;
	...
	assertNodeExecutedWith(nodeId: string, input: unknown): void;
...
export function createGraphExecutionDouble(): GraphExecutionCapabilities & {
	...
	const nodeExecutionHistory = new Map<
		string,
		Array<{
			input: unknown;
			output: unknown;
	...
	const mockNodeOutputs = new Map<string, unknown>();
	...
	mock.executeNode.mockImplementation(
		async (nodeId: string, input: unknown, _context: ExecutionContext) => {
			...
			const output = mockNodeOutputs.get(nodeId) || {
				...input,
				processed: true,
			};
	...
	mock.stream.mockImplementation(async function* (
		...
		for (const step of mockStreamSteps.length > 0
			? mockStreamSteps
			: [
					{
						nodeId: "start",
	...
	mock.updateExecutionState.mockImplementation(
		async (_executionId: string, _stateUpdate: Partial<HiveMindState>) => {
			// Mock implementation - in real system would update execution state
		},
	);

	// Configure error handling
	mock.handleNodeError.mockImplementation(
		async (_nodeId: string, _error: Error, _context: ExecutionContext) => {
	...
	mock.getExecutionMetrics.mockImplementation((_executionId: string) => {
		return {
			nodeExecutionTimes: {
				start: 10,
				process: 50,
	...
	const testHelpers: MockGraphExecutionHelpers = {
		...
		givenNodeExecutionReturns: (nodeId: string, output: unknown) => {
		...
		assertNodeExecutedWith: (nodeId: string, input: unknown) => {
			expect(mock.executeNode).toHaveBeenCalledWith(
				nodeId,
				input,
				expect.any(Object),
