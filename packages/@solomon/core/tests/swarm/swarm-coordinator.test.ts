...
const assertCalledWith = (mock: unknown, ...args: unknown[]) => {
	expect(mock).toHaveBeenCalledWith(...args);
};

const assertCalledOnce = (mock: unknown) => {
	expect(mock).toHaveBeenCalledTimes(1);
};

const _assertNeverCalled = (mock: unknown) => {
...
describe("SwarmCoordinator", () => {
	...
	describe("agent spawning", () => {
		it("should spawn worker agents based on task requirements", async () => {
			...
			testDoubles.queenAgent.__testHelpers.givenTaskAnalysisReturns({
				agentCount: 2,
			...
			testDoubles.workerAgent.__testHelpers.assertSpawnedWithType("programmer");
			testDoubles.workerAgent.__testHelpers.assertSpawnedWithType("tester");
		});
	...
	describe("consensus building", () => {
		it("should coordinate consensus for critical decisions", async () => {
			...
			// Set up test double behavior
			testDoubles.consensusEngine.__testHelpers.givenVotesReturn(mockVotes);
			testDoubles.consensusEngine.__testHelpers.givenConsensusResult({
				result: "approved",
				confidence: 0.85,
			...
			testDoubles.consensusEngine.__testHelpers.assertVotesCollectedFor(
				decision,
			);
		});
	});
	...
	describe("topology management", () => {
		it("should switch topology based on task complexity", async () => {
			...
			// Set up test double behavior
			testDoubles.topologyManager.__testHelpers.givenRecommendationReturns(
				"mesh",
			);
			...
			testDoubles.topologyManager.__testHelpers.assertTopologySwitched("mesh");
		});

		it("should handle topology switching failures gracefully", async () => {
			// Arrange
			const error = new Error("Topology switch failed");
			testDoubles.topologyManager.__testHelpers.givenSwitchTopologyFails(error);
			testDoubles.topologyManager.__testHelpers.givenCurrentTopology(
				"hierarchical",
			);
	...
	describe("error handling", () => {
		it("should handle agent failures with self-healing", async () => {
			...
			const spawnedWorkers =
				testDoubles.workerAgent.__testHelpers.getSpawnedInstances();
