...
describe("SwarmCoordinator Performance", () => {
	describe("with agent pool", () => {
		...
		it("should handle burst traffic efficiently", async () => {
			...
			const promises = tasks.map((task) =>
				coordinator.spawnAgentsForTask(task),
			);
			const results = await Promise.all(promises);
			const duration = Date.now() - startTime;

			// Assert
			const totalSpawned = results.reduce(
	...
	describe("consensus caching", () => {
		...
		it("should cache consensus decisions for performance", async () => {
			...
			expect(
				testDoubles.consensusEngine.calculateConsensus,
	...
	describe("comparative performance", () => {
		it("should perform better with optimizations enabled", async () => {
			...
			// Setup test doubles
			(
				testDoublesOptimized.queenAgent as any
			).__testHelpers.givenTaskAnalysisReturns({
				agentCount: 2,
				agentTypes: ["programmer", "tester"],
			});

			(
				testDoublesBasic.queenAgent as any
			).__testHelpers.givenTaskAnalysisReturns({
				agentCount: 2,
				agentTypes: ["programmer", "tester"],
			});
