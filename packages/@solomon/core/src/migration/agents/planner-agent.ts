...
export class PlannerAgent implements WorkerAgentCapabilities {
	...
	private setupGraph(): void {
		...
		this.graph.addNode(
			...
			this.analyzeRequirements.bind(this),
		);
		...
		this.graph.addConditionalEdges("optimize_plan", (state) => {
			...
			const hasResourceAllocation =
				Object.keys(state.resourceAllocation).length > 0;
	...
	private async assessRisks(
		...
		const risks = [
			...
			},
			{
				...
				impact: "high" as const,
				mitigation:
					"Cross-training team members and maintaining resource buffer",
			},
		];
	...
	private async optimizePlan(
		...
		const hasParallelizablePhases = state.plan.phases.some(
			(phase) =>
				phase.dependencies.length === 0 ||
				phase.dependencies.every(
					(dep) =>
						state.plan.phases.find((p) => p.id === dep)?.estimatedDuration ||
						0 < 16,
				),
		);
