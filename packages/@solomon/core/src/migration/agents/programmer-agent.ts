...
export const ProgrammerStateSchema = z.object({
	...
	refactoringSuggestions: z.array(
		z.object({
			file: z.string(),
			suggestion: z.string(),
...
export class ProgrammerAgent implements WorkerAgentCapabilities {
	...
	private setupGraph(): void {
		...
		this.graph.addConditionalEdges("check_quality", (state) => {
			...
			if (qualityMet) {
				return state.refactoringSuggestions.length > 0
					? "refactor"
					: "finalize";
			}
	...
	private async analyzeTask(
		...
		const messages = [
			new SystemMessage(
				"You are an expert programmer analyzing a development task.",
			),
			new HumanMessage(
	...
	private async writeTests(
		...
		const tests = state.implementation.files.map((file, _index) => ({
	...
	private async checkQuality(
		...
		// Calculate metrics
		const totalCoverage =
			state.implementation.tests.reduce((sum, test) => sum + test.coverage, 0) /
		...
		const suggestions =
			maintainability < 80
				? this.generateRefactoringSuggestions(state.implementation.files)
				: [];
	...
	private async refactorCode(
		...
		const updatedFiles = state.implementation.files.map((file) => {
			const suggestions = state.refactoringSuggestions.filter(
				(s) => s.file === file.path && s.automated,
			);
	...
	private calculateMaintainability(files: any[]): number {
		...
		const score = Math.max(
			0,
			Math.min(100, 100 - (avgLinesPerFile - 50) * 0.5),
		);
