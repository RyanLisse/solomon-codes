...
export {
	createProgrammerAgent,
	ProgrammerAgent,
	type ProgrammerState,
	ProgrammerStateSchema,
		...
		switch (agentType) {
			...
			case "programmer":
				return {
					...
					implementation: legacyState.code || {
						files: [],
						dependencies: [],
						tests: [],
					},
			...
			case "reviewer":
				return {
					...
					reviewFindings: legacyState.findings || {
						issues: [],
						metrics: {},
						suggestions: [],
					},
					reviewDecision: legacyState.decision || {
						verdict: "pending",
						confidence: 0,
						blockers: [],
						approvalConditions: [],
...
export class AgentRegistry {
	...
	static register(id: string, agent: any) {
		AgentRegistry.agents.set(id, agent);
	}

	static get(id: string) {
		return AgentRegistry.agents.get(id);
	}

	static remove(id: string) {
		return AgentRegistry.agents.delete(id);
	}

	static list() {
		return Array.from(AgentRegistry.agents.entries());
	}
	...
	static getByType(type: string) {
		return Array.from(AgentRegistry.agents.values()).filter(
