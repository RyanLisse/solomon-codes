...
export class ReviewerAgent implements WorkerAgentCapabilities {
	...
	private setupGraph(): void {
		...
		this.graph.addNode(
			...
			this.generateSuggestions.bind(this),
		);
		...
		this.graph.addConditionalEdges("make_decision", (state) => {
			if (
				state.reviewDecision.verdict === "needs_changes" &&
				state.reviewFindings.issues.length > 5
			) {
	...
	private async analyzeCode(
		state: ReviewerState,
	): Promise<Partial<ReviewerState>> {
		const messages = [
			new SystemMessage(
				"You are an expert code reviewer analyzing code for quality and correctness.",
			),
			new HumanMessage(
				`Review request: ${state.reviewRequest.description}\nFiles: ${state.reviewRequest.targetFiles.join(", ")}`,
			),
		];
	...
	private async checkQuality(
		...
		const metrics = {
			...
			security:
				100 -
				state.reviewFindings.issues.filter((i) => i.category === "security")
					.length *
					10,
			performance:
				100 -
				state.reviewFindings.issues.filter((i) => i.category === "performance")
					.length *
					5,
		};
	 ...
	 */
	private async performanceReview(
		state: ReviewerState,
	): Promise<Partial<ReviewerState>> {
	...
	private async generateSuggestions(
		...
		if (
			state.bestPractices.some(
				(bp) => bp.rule === "Documentation" && bp.violated,
			)
		) {
			suggestions.push({
	...
	private async makeDecision(
		...
		const criticalIssues = state.reviewFindings.issues.filter(
			(i) => i.severity === "critical",
		);
		const majorIssues = state.reviewFindings.issues.filter(
			(i) => i.severity === "major",
		);
	...
	private async generateReport(
		...
		const report = new AIMessage(
			"## Code Review Report\n\n" +
				...
				"### Recommendations\n" +
				state.reviewFindings.suggestions
					.map((s) => `- ${s.description}`)
					.join("\n"),
		);
	...
	async getMetrics(): Promise<any> {
		return {
			...
			criticalIssues: this.state.reviewFindings.issues.filter(
				(i) => i.severity === "critical",
			).length,
