...
import type { GraphMetadata } from "./base-graph";
...
export class SwarmGraphBuilder {
	...
	private async analyzeTask(
		state: SwarmGraphStateType,
	): Promise<Partial<SwarmGraphStateType>> {
	...
	private async optimizeTopology(
		...
		const optimalTopology =
			state.activeAgents.length > 5 ? "mesh" : "hierarchical";
